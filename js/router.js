...
const Router = {
  ...
  async init(mainContentElementId) {
    ...
    if (!this._mainContentElement) {
      console.error(`Router init: Elemen konten utama '${mainContentElementId}' tidak ditemukan.`);
      return;
    }
  ...
  'favorites': () => this._buildFavoritesPresenter(),
  'logout': () => this._handleLogoutRoute(),
    ...
    window.addEventListener('online', () => {
      console.log('[Router] Online kembali, mencoba sync outbox...');
      this._trySyncOutbox().then(() => {
  ...
  async _buildSavedStoriesPresenter() {
    ...
    await presenter.showSavedStories(true);

    return presenter;
  },

  async _buildFavoritesPresenter() {
  if (!this._authModel.isUserLoggedIn()) {
    this.navigateTo('login');
    return null;
  }
  ...
  const { default: FavoritesViewModule } = await import('./views/FavoritesView.js');
  const favoritesView = FavoritesViewModule;
  favoritesView.mainContent = this._mainContentElement;

  const { default: createFavoritesPresenterFactory } = await import('./presenters/FavoritesPresenter.js');
  const presenter = createFavoritesPresenterFactory(this._storyModel, this._authModel, favoritesView);

  await presenter.showFavorites();
  return presenter;
},
  ...
  _buildNotFoundPresenter() {
    if (this._mainContentElement) {
      this._mainContentElement.innerHTML = `
  ...
  navigateTo(routeName, forceRefresh = false) {
    const currentBaseRoute = window.location.hash.slice(1).split('/')[0];
    if (forceRefresh || currentBaseRoute !== routeName || window.location.hash.slice(1) !== routeName) {
  ...
  async _trySyncOutbox() {
    ...
    try {
      await this._loadStoryModel();
      if (!this._storyModel?.syncOutboxStories) return;
      ...
      console.error("Router: Gagal menjalankan sinkronisasi outbox otomatis:", e);
    }
  }
};
